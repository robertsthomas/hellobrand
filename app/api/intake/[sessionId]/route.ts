...
import { updateIntakeDraftForViewer } from "@/lib/intake/drafts";
...
export async function GET(
  ...
  try {
    ...
    debug.fail(error);
    const message = error instanceof Error ? error.message : "Could not load intake session.";
...
export async function PATCH(
  ...
  try {
    ...
    const session = await updateIntakeDraftForViewer(viewer, sessionId, {
      brandName: typeof body.brandName === "string" ? String(body.brandName) : null,
      campaignName: typeof body.campaignName === "string" ? String(body.campaignName) : null,
      notes: typeof body.notes === "string" ? String(body.notes) : null,
      pastedText: typeof body.pastedText === "string" ? String(body.pastedText) : null,
      inputSource:
    ...
    debug.fail(error);
    const message = error instanceof Error ? error.message : "Could not update intake draft.";

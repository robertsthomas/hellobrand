...
function PdfPageCanvas({
  ...
  useEffect(() => {
    ...
    async function renderPage() {
      ...
      try {
        ...
        const scale = Math.max(Math.min(containerWidth / baseViewport.width, 2.2), 0.4);
  ...
  return (
    <div
      ref={wrapperRef}
      className={`mx-auto w-full max-w-[900px] rounded-[20px] border bg-white p-3 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-4 ${
        isSearchMatch ? "border-[#d97706]/40 ring-2 ring-[#fbbf24]/30" : "border-black/8"
      }`}
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        {isSearchMatch ? (
          <span className="rounded-full bg-[#fff7ed] px-2.5 py-1 text-[10px] font-medium text-[#b45309]">
            Search match
          </span>
        ) : null}
      </div>
      ...
      {renderError ? (
        <div className="flex min-h-[240px] items-center justify-center rounded-[14px] border border-dashed border-black/10 bg-[#faf7f0] px-4 text-center text-sm text-muted-foreground">
          {renderError}
        </div>
      ) : (
        <canvas ref={canvasRef} className="mx-auto block max-w-full rounded-[12px]" />
      )}
    </div>
  );
}
...
function PdfDocumentPreview({
  ...
  if (isLoading) {
    return (
      <div className="flex h-full items-center justify-center bg-[#f6f1e8]">
  ...
  if (errorMessage || !pdfDocument) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 bg-[#f6f1e8] px-6 text-center">
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="sticky top-0 z-10 border-b border-black/8 bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <Input
                ...
                className="h-10 border-black/10 bg-white pl-10 text-sm"
              />
            </div>
            <MatchNavigator
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-[#efe8dc] px-3 py-4 sm:px-5 sm:py-5">
...
function TextDocumentPreview({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-[#f8f4ed] px-4 py-4 sm:px-6">
        <div className="mx-auto max-w-[900px] rounded-[20px] border border-black/8 bg-white p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-5">

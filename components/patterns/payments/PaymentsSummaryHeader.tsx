  ...
  return (
    <div className="mt-2 space-y-1">
      ...
      <p className="text-xs text-muted-foreground">
        {summary.breakdown.map((entry) => formatCurrency(entry.amount, entry.currency)).join(" · ")}
      </p>
    </div>
  );
}
    ...
    <section className="space-y-6 border-b border-black/8 pb-8">
      <div className="max-w-3xl">
        ...
        <p className="mt-4 text-base leading-7 text-muted-foreground sm:text-lg sm:leading-8">
          Track every workspace payment in one place, see whether an invoice exists, and open each
          workspace for the full payment split and attachment details.
        </p>
      </div>

      <div
        data-guide="payments-summary"
        className="grid gap-6 border-t border-black/8 pt-5 sm:grid-cols-3"
      >
        <div>

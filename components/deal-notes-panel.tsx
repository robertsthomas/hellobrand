...
export function DealNotesPanel({ dealId, notes }: { dealId: string; notes: string | null }) {
  const sanitizedNotes = sanitizePlainTextInput(notes);

  return (
    <form
      action={saveDealNotesAction}
      className="grid gap-6 border border-black/8 bg-white p-6 dark:border-white/10 dark:bg-card"
    >
      <div>
        ...
        <p className="mt-2 text-sm text-black/60 dark:text-white/65">
          Keep your creator-side context here, like negotiation reminders or delivery prep notes.
        </p>
      </div>
      ...
      <textarea
        className="min-h-40 border border-black/10 bg-white px-4 py-4 text-sm text-foreground outline-none transition focus:border-ring dark:border-white/12 dark:bg-white/[0.03] dark:focus:border-ring"
        name="notes"

...
function DocumentSheet({ reducedMotion, active }: { reducedMotion: boolean; active: boolean }) {
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        animate={
          reducedMotion ? { opacity: 0.5 } : active ? { y: ["-105%", "105%"] } : { y: "-105%" }
        }
        transition={{
          duration: 2.4,
          ease: "linear",
          ...
          >
            {row.highlighted ? (
              <motion.div
                ...
                animate={reducedMotion ? { opacity: 0.85 } : { x: ["-100%", "100%"] }}
                transition={{
                  duration: 1.45,
                  delay: index * 0.12,
                  repeat: Infinity,
        ...
        animate={
          reducedMotion
            ...
            : active
              ...
              : { boxShadow: "0 0 0 rgba(127,209,173,0)" }
        }
      ...
      >
        ...
        <motion.div
          ...
          animate={
            reducedMotion ? { opacity: 1 } : active ? { opacity: [1, 0.4, 1] } : { opacity: 1 }
          }
...
function SummaryPanel({ reducedMotion, active }: { reducedMotion: boolean; active: boolean }) {
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        animate={
          reducedMotion ? { opacity: 0.7 } : active ? { opacity: [0.4, 1, 0.4] } : { opacity: 0.4 }
        }
      ...
      <div className="mb-3 rounded-xl border border-white/10 bg-white/[0.04] p-3 sm:mb-4 sm:p-4">
        ...
        >
          Brand agreement includes deliverables, payment timing, a 90-day usage window, and one
          exclusivity restriction to review.
        </motion.p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/10 sm:h-9 sm:w-9">
                <Icon
                  className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${item.accentClassName}`}
                  strokeWidth={1.5}
                />
              </div>
              <div className="min-w-0">
                <div className="text-[13px] font-medium text-white sm:text-sm">{item.label}</div>
      ...
      >
        <motion.div
          ...
          animate={reducedMotion ? { opacity: 0.35 } : active ? { x: ["0%", "520%"] } : { x: "0%" }}
          transition={{
            duration: 2.6,
            repeat: Infinity,
            ease: "linear",
...
export function DocumentScanShowcase() {
  ...
  const isVisible = useInView(ref, { once: false, margin: "-100px" });

  return (
    <section ref={ref} className="relative overflow-hidden bg-[#10221e] dark:bg-[#0d1714]">
      ...
      >
        <div className="mb-6 text-center sm:mb-8 lg:mb-10">
          ...
          >
            Upload a contract, get an instant breakdown of clauses, risks, and payment terms.
          </motion.p>
        </div>
        ...
        <div className="flex flex-col items-center gap-5 lg:grid lg:grid-cols-[1fr_auto_1fr] lg:items-center lg:gap-10 xl:gap-14">
          <div className="flex w-full justify-center lg:justify-end">
            <DocumentSheet reducedMotion={Boolean(reducedMotion)} active={isVisible} />
          </div>

          <ConnectorArrow />

          <div className="flex w-full justify-center lg:justify-start">
            <SummaryPanel reducedMotion={Boolean(reducedMotion)} active={isVisible} />
          </div>
        </div>
      </motion.div>
    </section>
